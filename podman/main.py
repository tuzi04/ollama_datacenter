...
def restart_ollama():
    os.system("podman stop ollama") # It makes an error message when it runs for the first time.
    time.sleep(20)
    os.sytem(f"podman run --rm -d --init --security-opt=label=disable --mount=type=bind,src=.,dst=/mnt/workspace --device=nvidia.com/gpu=0 --hooks-dir=/usr/share/containers/oci/hooks.d/ -v ollama:/root/.ollama -p {sys.argv[2]}:11434 --name ollama ollama/ollama")
    client = Client(
